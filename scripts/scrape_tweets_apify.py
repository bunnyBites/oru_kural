# DEPRECATED: Apify-based scraper — kept for historical backfill only.
# Not reliable for scheduled runs. Use scrape_tweets.py (X API v2) instead.
# Requires APIFY_API_KEY in .env

"""
Scrape tweets mentioning @CMOTamilnadu via Apify and upsert to Supabase.
...
TWITTER_COOKIES: list = json.loads(os.environ.get("TWITTER_COOKIES", "[]"))

ACTOR_ID = "apidojo~tweet-scraper"
SEARCH_QUERY = "@CMOTamilnadu"
MAX_ITEMS = 500
...
async def start_apify_run(client: httpx.AsyncClient) -> tuple[str, str]:
    ...
    resp = await client.post(
        ...
        json={
            ...
            "maxItems": MAX_ITEMS,
            **({"cookies": TWITTER_COOKIES} if TWITTER_COOKIES else {}),
        },
        timeout=30,
    )
...
def map_item(item: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        ...
        print(f"  skipping malformed item: {exc} — keys: {list(item.keys())}")
        return None
...
async def main() -> None:
    ...
    async with httpx.AsyncClient() as client:
        ...
        if raw_items:
            ...
            print(f"  first raw item (truncated): {json.dumps(raw_items[0])[:300]}")
        real_items = [item for item in raw_items if "noResults" not in item]
        print(f"mapping {len(real_items)} real items ({len(raw_items) - len(real_items)} noResults entries ignored)…")
        rows = [row for item in real_items if (row := map_item(item)) is not None]
        skipped = len(real_items) - len(rows)
